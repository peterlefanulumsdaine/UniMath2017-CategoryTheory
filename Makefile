...
############################################
# The current library master name
MAINNAME := UniMathBham2017CT
# The packages, listed in order by dependency:
PACKAGES += Auxiliary
PACKAGES += EatsAndDats
############################################
...
PACKAGE_FILES := $(patsubst %, UniMathBham2017CT/%/package/files, $(PACKAGES))
...
$(foreach P,$(PACKAGES),$(eval TAGS-$P: $(filter UniMathBham2017CT/$P/%,$(VFILES)); etags -o $$@ $$^))
$(VFILES:.v=.vo) : # $(COQBIN)coqc
TAGS : $(PACKAGE_FILES) $(VFILES); etags $(COQDEFS) $(VFILES)
FILES_FILTER := grep -vE '^[[:space:]]*(\#.*)?$$'
$(foreach P,$(PACKAGES),$(eval $P: $(shell <UniMathBham2017CT/$P/package/files $(FILES_FILTER) |sed "s=^\(.*\)=UniMathBham2017CT/$P/\1o=" )))
install:all
coqwc:; coqwc $(VFILES)
lc:; wc -l $(VFILES)
lcp:; for i in $(PACKAGES) ; do echo ; echo ==== $$i ==== ; for f in $(VFILES) ; do echo "$$f" ; done | grep "UniMathBham2017CT/$$i" | xargs wc -l ; done
wc:; wc -w $(VFILES)
admitted: 
...
.coq_makefile_input: $(PACKAGE_FILES) $(UMAKEFILES)
	...
	echo ;\
	echo '-Q UniMathBham2017CT UniMathBham2017CT' ;\
	echo ;\
	for i in $(PACKAGES) ;\
	do <UniMathBham2017CT/$$i/package/files $(FILES_FILTER) |sed "s=^=UniMathBham2017CT/$$i/="  ;\
	done ;\
	echo ;\
...
clean::
	...
	find UniMathBham2017CT \( -name .\*.aux -o -name \*.glob -o -name \*.v.d -o -name \*.vo \) -delete
	find UniMathBham2017CT -type d -empty -delete
...
# The isolated bug will appear in this file, in the UniMathBham2017CT directory:
...
$(LATEXDIR)/coqdoc.sty $(LATEXDIR)/helper.tex : $(VFILES:.v=.glob) $(VFILES)
	$(COQDOC) -Q UniMathBham2017CT UniMathBham2017CT $(COQDOC_OPTIONS) $(COQDOCLATEXOPTIONS) $(VFILES) -o $@
